...
test_github_actions_release_workflow_is_disabled() {
  if [[ -e "${SOURCE_RELEASE_WORKFLOW}" ]]; then
    fail "GitHub Actions release workflow must remain disabled: ${SOURCE_RELEASE_WORKFLOW}"
  fi
}
